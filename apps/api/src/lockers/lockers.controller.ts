...
} from '../allocations/allocation-integrity';
...
export class LockersController {
  ...
  async list() {
    ...
    return lockers.map((l) => {
      const active = l.allocations[0] ?? null;
      const status = active
        ? 'OCCUPIED'
      ...
      return {
        ...
        lab: l.lab,
        status,
        currentUserId: active?.userId ?? null,
        currentUserName: active?.user?.name ?? null,
      };
    });
  }
  ...
  async create(@Req() req: any, @Body() dto: CreateLockerDto) {
    ...
    if (![2, 3, 5, 6, 7, 8].includes(floor)) {
      throw new BadRequestException(
        'Andar inválido (permitidos: 2, 3, 5, 6, 7, 8).',
      );
    }
    ...
    try {
      ...
      throw new BadRequestException(
        'Já existe uma chave com esse número nesse andar.',
      );
    }
  }
  ...
  async update(
    ...
    if (dto.lab !== undefined)
      data.lab = dto.lab === null ? null : String(dto.lab).trim() || null;
    ...
    try {
      ...
      throw new BadRequestException(
        'Conflito: já existe uma chave com esse número nesse andar.',
      );
    }
  }
  ...
  async updateStatus(
    ...
    await this.prisma.$transaction((tx) => reconcileLockerState(tx, id));
    ...
    const nextCurrentUserId =
      status === 'OCCUPIED' ? locker.currentUserId : null;

    // Se tentar colocar em manutenção, só permite se estiver livre e sem alocação ativa
    if (status === 'MAINTENANCE') {
      if (locker.status !== 'FREE') {
        throw new BadRequestException(
          'Só é possível marcar como Manutenção quando o armário está Livre.',
        );
      }

      const active = await findActiveAllocationForLocker(
        this.prisma,
        locker.id,
      );
      ...
      if (active) {
        throw new BadRequestException(
          'Este armário está com alocação ativa. Registre a devolução antes de marcar Manutenção.',
        );
      }
    }
    // regra: não pode colocar FREE se existe alocação ativa (evita inconsistência)
    if (status === 'FREE') {
      const active = await findActiveAllocationForLocker(this.prisma, id);
      if (active)
        throw new BadRequestException(
          'Não pode marcar como Livre: existe alocação ativa.',
        );
    }
  ...
  async remove(@Req() req: any, @Param('id') id: string) {
    ...
    if (active)
      throw new BadRequestException(
        'Não é possível excluir: armário possui alocação ativa.',
      );
    ...
    if (anyHistory)
      throw new BadRequestException(
        'Não é possível excluir: este armário já possui histórico de utilização.',
      );
