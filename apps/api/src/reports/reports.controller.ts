...
export class ReportsController {
  ...
  async auditPdf(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="relatorio_auditoria.pdf"`,
    );
    ...
    doc.fontSize(20).text('Sistema de Gestão de Armários', 120, 50);

    doc.fontSize(12).fillColor('#555').text('PROPPGI / UFCSPA', 120, 75);
    ...
    const period = `Período: ${from ? from.toLocaleDateString('pt-BR') : 'Início'} até ${
    ...
    for (const l of logs) {
      ...
      doc
        ...
        .fillColor('#444')
        .text(
          `${actionLabel(l.action)}  •  ${l.entity}${l.entityId ? ` (${l.entityId.slice(0, 8)})` : ''}`,
        );

      doc.fillColor('#000').text(l.details);
